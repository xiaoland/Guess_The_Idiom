...
var data = [][]string{
	...
	{"对牛弹琴", "http://dbp-resource.gz.bcebos.com/509b8811-c1d4-238d-5a0e-1f1b319a9e4b/%E5%AF%B9%E7%89%9B%E5%BC%B9%E7%90%B4.jpg?authorization=bce-auth-v1%2Fa4d81bbd930c41e6857b989362415714%2F2018-06-26T15%3A17%3A06Z%2F-1%2F%2Fd602b8077505b0505532d48c13e8a07ef16d842fa62a94c58b13c3dab5cac852"},
	...
	{"坐吃山空", "http://dbp-resource.gz.bcebos.com/509b8811-c1d4-238d-5a0e-1f1b319a9e4b/%E5%9D%90%E5%90%83%E5%B1%B1%E7%A9%BA.jpg?authorization=bce-auth-v1%2Fa4d81bbd930c41e6857b989362415714%2F2018-06-27T05%3A32%3A57Z%2F-1%2F%2F8a83010d48f201c71b1049e02b5b145ae13fefba0244f197ac2ce8ccafbf4349"},
...
func main() {
	...
	bot.AddIntentHandler("idiom", startGame)
	bot.AddIntentHandler("idiom_answer", answer)
	bot.AddIntentHandler("nidiom", nextMission)
	bot.AddIntentHandler("answerhelp", helpAnswer)
	bot.AddIntentHandler("c_idiom", nextRound)
	bot.AddIntentHandler("ai.dueros.common.default_intent", defaultIntent)

	application := dueros.Application{AppId: "509b8811-c1d4-238d-5a0e-1f1b319a9e4b", Handler: bot.Handler} // 添加web容器
...
func startGame(bot *dueros.Bot, request *model.IntentRequest) {
	pos := rand.Intn(len(data))

	bot.Session.SetAttribute("pos", strconv.Itoa(pos))
	bot.Session.SetAttribute("round_error", "0")   // 单回合错误次数
	bot.Session.SetAttribute("mission_right", "0") // 单轮回答正确次数
	bot.Session.SetAttribute("round_num", "1")     // 第几关
	bot.Session.SetAttribute("mission_num", "1")   // 第几轮

	bot.Response.Tell("好的，让我们进入第1轮")
	bot.Response.HoldOn()

	showImage(bot, pos)
}

func answer(bot *dueros.Bot, request *model.IntentRequest) {

	bot.Response.HoldOn()

	pos, err := strconv.Atoi(bot.Session.GetAttribute("pos"))

	if err != nil { // 处理未说开始的情况
		launchRequest(bot, nil)
		return
	}

	result := builtResult(request.Dialog.GetSlotValue("idiom"))

	rightAnswer := data[pos][0]

	if result == rightAnswer {
		handlerRight(bot, request)
	} else {
		handlerWrong(bot, request)
	}
}

func handlerRight(bot *dueros.Bot, request *model.IntentRequest) {

	bot.Session.SetAttribute("round_error", "0") // 单回合错误次数重置

	roundNum, _ := strconv.Atoi(bot.Session.GetAttribute("round_num"))
	missionRight, _ := strconv.Atoi(bot.Session.GetAttribute("mission_right"))
	missionNum, _ := strconv.Atoi(bot.Session.GetAttribute("mission_num"))

	if roundNum < 10 { // 正常下一个
		pos := rand.Intn(len(data))
		bot.Session.SetAttribute("round_num", strconv.Itoa(roundNum+1))         // 下一关
		bot.Session.SetAttribute("mission_right", strconv.Itoa(missionRight+1)) // 单轮回答正确次数
		bot.Session.SetAttribute("pos", strconv.Itoa(pos))
		bot.Response.Tell("恭喜你答对了，您已经闯过了第" + strconv.Itoa(roundNum) + "关，加油！请看第" + strconv.Itoa(roundNum+1) + "关题目")
	...
	// 开始结算界面
	bodyTemp := template.NewBodyTemplate1()
	bodyTemp.SetBackgroundImageUrl("http://dbp-resource.gz.bcebos.com/509b8811-c1d4-238d-5a0e-1f1b319a9e4b/%E5%85%A8%E5%BF%83%E6%8A%95%E5%85%A5.jpg?authorization=bce-auth-v1%2Fa4d81bbd930c41e6857b989362415714%2F2018-06-27T05%3A30%3A00Z%2F-1%2F%2F65d7fb04d32c2d64abb134ad8be345c4bd81a32bdfe8071dc56d19da7c9de5f0")
	bodyTemp.SetPlainContent("你太棒了，本轮一共十道题，你答对了" + strconv.Itoa(missionRight+1) + "道题目，说出“开始下一轮”即可进入第" + strconv.Itoa(missionNum) + "轮，需要退出请说：“退出”。")
	directive := display.NewRenderTemplate(bodyTemp)
	bot.Response.Command(directive)
	bot.Response.Tell("你太棒了，本轮一共十道题，你答对了" + strconv.Itoa(missionRight+1) + "道题目，说出“开始下一轮”即可进入第" + strconv.Itoa(missionNum) + "轮，需要退出请说：“退出”。")
}

func handlerWrong(bot *dueros.Bot, request *model.IntentRequest) {

	roundNum, _ := strconv.Atoi(bot.Session.GetAttribute("round_num"))
	missionRight, _ := strconv.Atoi(bot.Session.GetAttribute("mission_right"))
	missionNum, _ := strconv.Atoi(bot.Session.GetAttribute("mission_num"))
	pos, _ := strconv.Atoi(bot.Session.GetAttribute("pos"))
	roundError, _ := strconv.Atoi(bot.Session.GetAttribute("round_error"))

	if roundError >= 2 && roundNum < 10 {
		bot.Response.Tell("好遗憾，错误次数太多，正确答案是：" + data[pos][0] + "，不要气馁，让我们继续吧")
	...
	if roundError >= 2 && roundNum >= 10 {
		...
		bodyTemp := template.NewBodyTemplate1()
		bodyTemp.SetBackgroundImageUrl("http://dbp-resource.gz.bcebos.com/509b8811-c1d4-238d-5a0e-1f1b319a9e4b/%E5%85%A8%E5%BF%83%E6%8A%95%E5%85%A5.jpg?authorization=bce-auth-v1%2Fa4d81bbd930c41e6857b989362415714%2F2018-06-27T05%3A30%3A00Z%2F-1%2F%2F65d7fb04d32c2d64abb134ad8be345c4bd81a32bdfe8071dc56d19da7c9de5f0")
		bodyTemp.SetPlainContent("你太棒了，本轮一共十道题，你答对了" + strconv.Itoa(missionRight) + "道题目，说出“开始下一轮”即可进入第" + strconv.Itoa(missionNum) + "轮，需要退出请说：“退出”。")
		directive := display.NewRenderTemplate(bodyTemp)
		bot.Response.Command(directive)
		bot.Response.Tell("你太棒了，本轮一共十道题，你答对了" + strconv.Itoa(missionRight) + "道题目，说出“开始下一轮”即可进入第" + strconv.Itoa(missionNum) + "轮，需要退出请说：“退出”。")

	}

	roundError += 1
	bot.Response.Tell("你已经答错了" + strconv.Itoa(roundError) + "次了，再努力想想吧，需要帮助可以说，我需要帮助")
	bot.Session.SetAttribute("round_error", strconv.Itoa(roundError))
}

func nextMission(bot *dueros.Bot, request *model.IntentRequest) {

	bot.Response.HoldOn()

	missionNum, err := strconv.Atoi(bot.Session.GetAttribute("mission_num"))
	if err != nil {
		launchRequest(bot, nil)
	}
	...
	bot.Session.SetAttribute("mission_right", "0")                      // 单轮回答正确次数
	bot.Session.SetAttribute("round_num", "1")                          // 第几关
	bot.Session.SetAttribute("mission_num", strconv.Itoa(missionNum+1)) // 第几轮
	bot.Response.Tell("好的，让我们进入第" + strconv.Itoa(missionNum+1) + "轮")
	showImage(bot, pos)
}

func helpAnswer(bot *dueros.Bot, request *model.IntentRequest) {
	...
	switch rand.Intn(3) {
	case 0:
		bot.Response.Tell("诶呀，成语躲起来了，加油想一想吧")
	case 1:
		bot.Response.Tell("上官，答案的第一个字是" + data[pos][0][0:3])
	case 2:
		bot.Response.Tell("上官，答案的前两个字是" + data[pos][0][0:6])
	}
}

func nextRound(bot *dueros.Bot, request *model.IntentRequest) {
	...
	if roundNum < 10 { // 正常下一个
		...
		bot.Session.SetAttribute("round_num", strconv.Itoa(roundNum+1)) // 下一关
		bot.Session.SetAttribute("pos", strconv.Itoa(pos))
		bot.Response.Tell("请看第" + strconv.Itoa(roundNum+1) + "关题目")
		showImage(bot, pos)
		return
	}
	...
	directive := display.NewRenderTemplate(bodyTemp)
	bot.Response.Command(directive)
	bot.Response.Tell("你太棒了，本轮一共十道题，你答对了" + strconv.Itoa(missionRight+1) + "道题目，说出“开始下一轮”即可进入第" + strconv.Itoa(missionNum+1) + "轮，需要退出请说：“退出”。")
}

func defaultIntent(bot *dueros.Bot, request *model.IntentRequest) {
	...
	roundError += 1
	bot.Session.SetAttribute("round_error", strconv.Itoa(roundError))
	bot.Response.Tell("再努力想想吧，需要帮助可以说，我需要帮助")
}

func showImage(bot *dueros.Bot, pos int) {
	imageCard := card.NewImageCard()
	imageCard.AddItem(data[pos][1], "")

	bot.Response.DisplayCard(imageCard)
}

func builtResult(data string) string {
	result := &Result{}
